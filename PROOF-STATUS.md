...
| DeepSeek function-calling bridge | PASS WITH LIMITS | `node --test test/deepseek-function-client.test.mjs` — 3/3; fake-client coverage proves one structured call, usage readback, missing-key rejection, read-only enforcement, and prose/multiple-call rejection | No DeepSeek request has been made from this checkout because no local key is configured; this is a function-calling bridge, not native DeepSeek MCP |
